...
import { Button, Card, Progress } from "@/components/shared/ui";
import {
  ...
  return (
    <main className="page-shell flex min-h-[calc(100svh-7rem)] items-center justify-center py-8">
      <Card className="w-full max-w-lg text-center">
        <div className="mx-auto mb-5 flex size-14 items-center justify-center rounded-2xl bg-white/8 text-primary">
          {icon}
        </div>
...
function UserHeader() {
  return (
    <header className="border-b border-white/8 bg-[#090b11]/80 backdrop-blur-2xl">
      <div className="page-shell flex min-h-20 items-center justify-between gap-3">
        <BrandLockup href="/u" />
        <span className="hidden items-center gap-2 text-xs text-white/50 sm:flex">
...
function Checklist() {
    ...
    return (
      <main className="page-shell py-8 sm:py-12">
        <Card className="mx-auto max-w-2xl text-center">
          ...
          <div className="mt-7 flex items-start gap-3 rounded-2xl border border-orange-300/15 bg-orange-300/6 p-4 text-left">
            <LockKeyhole className="mt-0.5 size-5 shrink-0 text-orange-200" />
  ...
  return (
    <>
      <main className="page-shell pb-32 pt-7 sm:pt-10">
        <div className="mx-auto max-w-3xl">
          ...
          <Card className="mb-5">
            ...
            />
            <div className="mt-4 flex items-start gap-3 rounded-xl bg-emerald-300/6 p-3 text-xs leading-5 text-emerald-100/75">
            ...
            {request.documents.map((document, index) => (
              <Card key={document.id}>
                <div className="mb-4 flex items-start gap-3">
                  <span className="flex size-8 shrink-0 items-center justify-center rounded-xl bg-primary/12 text-sm font-bold text-orange-200">
                    {index + 1}
                  </span>
                  <div>
          ...
          {generationError ? (
            <p
              role="alert"
              className="mt-5 rounded-xl border border-red-300/20 bg-red-300/8 p-4 text-sm text-red-100"
            >
              {generationError}
            </p>
          ) : null}
      ...
      <div className="safe-bottom fixed inset-x-0 bottom-0 z-20 border-t border-white/10 bg-[#090b11]/90 p-4 backdrop-blur-2xl">
        <div className="mx-auto flex max-w-3xl items-center justify-between gap-4">
          <div className="hidden sm:block">
            ...
            <p className="text-xs text-white/45">Nothing will be uploaded.</p>
          </div>
          <Button
